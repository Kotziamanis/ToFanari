...
from pdf_ops import Marker as PdfOpsMarker
...
class PDFMarkerApp:
    def __init__(
        ...
        *,
        on_saved_pdf: Optional[Callable[[str], None]] = None,
    ) -> None:
        self.root = root
        self._embedded = isinstance(root, tk.Toplevel)
        self.on_saved_pdf = on_saved_pdf
    ...
    def _scoped(self, e: tk.Event, fn: Callable[[tk.Event], None]) -> None:
        """When embedded in a Toplevel, ignore events from other windows."""
        try:
    ...
    def _scoped_wrapper(self, fn: Callable[[tk.Event], None]) -> Callable[[tk.Event], None]:
        return lambda e: self._scoped(e, fn)

    def seed_from_pdf_ops_markers(self, markers: List[PdfOpsMarker]) -> None:
        """
        Replace in-memory markers from main-app pdf_ops.Marker list (after PDF is open).
        Book order: page asc, y asc, x asc (same as compact).
        """
        if self.doc is None:
            return
        ...
        for m in active:
            ...
            new_by_page.setdefault(em.page_no, []).append(em)
    ...
    def _bind_events(self) -> None:
        ...
        S = self._scoped_wrapper
        if self._embedded:
            self.root.bind_all("<MouseWheel>", S(self.on_mouse_wheel))
            self.root.bind_all("<Button-4>", S(self.on_mouse_wheel_linux))
            self.root.bind_all("<Button-5>", S(self.on_mouse_wheel_linux))
            self.root.bind_all("<Delete>", S(self.on_delete_key))
            self.root.bind_all("<Escape>", S(self.on_escape))
            self.root.bind_all("<Control-z>", S(self.on_ctrl_z))
            self.root.bind_all("<Prior>", S(self.on_page_up_key))
            self.root.bind_all("<Next>", S(self.on_page_down_key))
            self.root.bind_all("<Home>", S(self.on_home_key))
            self.root.bind_all("<End>", S(self.on_end_key))
        else:
    ...
    def save_pdf_with_markers(self) -> None:
        ...
        try:
            ...
            if self.on_saved_pdf is not None:
                try:
                    self.on_saved_pdf(path)
                except Exception:
                    pass
