...
MARKER_CHAR = "\u25A0"  # ■ U+25A0 - ακριβώς αυτό που ψάχνει το ToFanari app

class PDFMarkerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PDF Marker Tool - Προσθήκη ■")
        ...
        # Info
        info = tk.Label(root, text="👆 Κλίκ στη σελίδα για να προσθέσεις ■ marker",
    ...
    def render_page(self):
        ...
        if self.current_page in self.markers:
            for (px, py) in self.markers[self.current_page]:
                cx = px * self.zoom
                cy = py * self.zoom
    ...
    def save_pdf(self):
        ...
        # Βρες font path που υποστηρίζει U+25A0
        font_paths = [
            "C:/Windows/Fonts/arial.ttf",
            "C:/Windows/Fonts/cour.ttf",
            "C:/Windows/Fonts/seguisym.ttf",  # Segoe UI Symbol - υποστηρίζει ■
        ...
        for fp in font_paths:
            if os.path.exists(fp):
                font_path = fp
                # Segoe UI Symbol είναι η καλύτερη επιλογή
                if "segui" in fp:
                    break
        ...
        for page_num, positions in self.markers.items():
            ...
            for (px, py) in positions:
                ...
                if font_path:
                    # Χρήση system font που υποστηρίζει U+25A0
                    page.insert_text(
                        point,
                        MARKER_CHAR,
                        fontsize=14,
        ...
        # Επαλήθευση ότι το ■ αναγνωρίζεται
