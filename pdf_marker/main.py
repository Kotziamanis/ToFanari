...
from marker_recovery import (
    apply_markers_to_fitz_document,
    guess_source_pdf_beside_json,
    recover_marked_pdf_from_json_files,
)

# PDF Marker only — do not import app.py, config.py, or other Tofanari Main Tool modules.
...
class PDFMarkerApp:
    ...
    def _build_ui(self) -> None:
        ...
        btn_recover_pdf = tk.Button(
            row1,
            text="Recover PDF from JSON",
            command=self.recover_pdf_from_json_mistake,
            bg="#8b6914",
            fg="white",
            font=("Arial", 10, "bold"),
            padx=8,
        )
        btn_recover_pdf.pack(side=tk.LEFT, padx=4)
        ...
        info = tk.Label(
            self.root,
            text="Continuous scroll: wheel/PageUp/PageDown/Home/End | Click to place ■ | Save PDF embeds markers + markers file | Save JSON optional | Recover PDF from JSON if you saved JSON only by mistake | Right-click to delete",
            bg="#2b1a1a",
            fg="#FFD700",
            font=("Arial", 11),
        )
    ...
    def open_pdf_path(self, path: str) -> None:
        ...
        self.filepath = path
        try:
            self.root.title(f"PDF Marker — {os.path.basename(path)}")
        except Exception:
            pass
    ...
    def _apply_markers_to_fitz_doc(
        ...
        apply_markers_to_fitz_document(doc, items)

    def recover_pdf_from_json_mistake(self) -> None:
        """
        Build a marked PDF from a saved markers JSON + the original book PDF.
        Use this if Save JSON was used by mistake instead of Save PDF.
        """
        json_path = filedialog.askopenfilename(
            filetypes=[("Markers JSON", "*.json"), ("All files", "*.*")],
            title="Select markers JSON file",
        )
        if not json_path:
            return

        initial_dir = os.path.dirname(os.path.abspath(json_path))
        guess = guess_source_pdf_beside_json(json_path)

        open_opts: dict = {
            "filetypes": [("PDF files", "*.pdf"), ("All files", "*.*")],
            "initialdir": initial_dir,
            "title": "Select the original PDF (same book, without markers)",
        }
        if guess:
            open_opts["initialfile"] = os.path.basename(guess)
        src = filedialog.askopenfilename(**open_opts)
        if not src:
            return

        base = os.path.splitext(os.path.basename(json_path))[0]
        if base.endswith("_markers"):
            out_initial = f"{base[: -len('_markers')]}_marked.pdf"
        else:
            out_initial = f"{base}_marked.pdf"
        dest = filedialog.asksaveasfilename(
            defaultextension=".pdf",
            filetypes=[("PDF files", "*.pdf")],
            initialdir=initial_dir,
            initialfile=out_initial,
            title="Save recovered PDF with markers",
        )
        if not dest:
            return

        try:
            count, warnings = recover_marked_pdf_from_json_files(json_path, src, dest)
            msg = (
                f"Recovered PDF with {count} marker(s).\n\nSaved to:\n{dest}\n\n"
                "You can open that PDF in this tool to continue editing if needed."
            )
            if warnings:
                msg += "\n\n" + "\n".join(warnings)
            messagebox.showinfo("Recover PDF from JSON", msg)
            self._set_status("Recovered PDF from JSON")
        except Exception as e:
            messagebox.showerror("Recover PDF from JSON", f"Could not build PDF:\n{e}")
            self._set_status("Recovery failed")
    ...
    def save_pdf_with_markers(self) -> None:
        ...
        try:
            ...
            # Treat the output file as the current document for marker state: sidecar JSON
            # must live next to the saved PDF (not only the originally opened path), so
            # autosave, Save JSON, and reopening this PDF all use the same markers.
            # We do not reload self.doc from the saved file — that PDF embeds marker
            # graphics; reloading would duplicate the on-screen overlay.
            saved_abs = os.path.abspath(path)
            self.filepath = saved_abs
            try:
                self.root.title(f"PDF Marker — {os.path.basename(saved_abs)}")
            except Exception:
                pass
            ...
            try:
                ...
                messagebox.showwarning(
                    "Markers file",
                    "PDF was saved, but writing the markers file next to it failed:\n"
                    f"{json_exc}\n\n"
                    "You can still edit markers in this session. Use **Save JSON** to save positions.",
                )

            self._set_status("PDF saved")
            msg = f"PDF saved to:\n{path}"
            if json_path:
                msg += f"\n\nMarkers saved to:\n{json_path}\n\nYou can keep editing, deleting, or adding markers; use Save PDF again when ready."
